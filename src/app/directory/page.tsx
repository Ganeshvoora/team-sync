...
export default function DirectoryPage() {
  ...
  if (loading) {
    return (
      <AdminLayout title="Employee Directory" subtitle="Find and Connect">
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-white mx-auto mb-4"></div>
            <div className="text-white text-xl mb-2">Loading directory...</div>
          </div>
        </div>
      </AdminLayout>
    )
  ...
  return (
    <AdminLayout title="Employee Directory" subtitle="Find and Connect with Team Members">
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredEmployees.map((employee) => (
            <Card key={employee.id} className="bg-white/10 backdrop-blur-xl border-white/20 hover:bg-white/15 transition-all duration-200">
              <CardContent className="p-6">
                ...
                <div className="flex space-x-2">
                  <Button
                    ...
                    className="flex-1 bg-blue-500/20 text-blue-300 hover:bg-blue-500/30 border border-blue-400/30"
                  >
                  ...
                  {canAssignTaskTo(employee) && (
                    <Button
                      ...
                      className="flex-1 bg-green-500/20 text-green-300 hover:bg-green-500/30 border border-green-400/30"
                    >
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center space-x-3">
              ...
              <div>
                <h3 className="text-xl">{selectedEmployee?.name}</h3>
                <p className="text-sm text-gray-300">{selectedEmployee?.role.name} • {selectedEmployee?.department?.name}</p>
              </div>
            </DialogTitle>
          </DialogHeader>
                ...
                <CardContent className="space-y-3">
                  ...
                  {selectedEmployee.contactNumber && (
                    <div>
                      ...
                      <p className="text-white">{selectedEmployee.contactNumber}</p>
                    </div>
                  )}
                  ...
                  {selectedEmployee.employeeId && (
                    <div>
                      ...
                      <p className="text-white">{selectedEmployee.employeeId}</p>
                    </div>
                  )}
                </CardContent>
              </Card>
              ...
              <Card className="bg-white/5 border-white/10">
                ...
                <CardContent className="space-y-3">
                  <div>
                    ...
                    <p className="text-white">{selectedEmployee.role.name}</p>
                  </div>
                  <div>
                    <span className="text-gray-400 text-sm">Department:</span>
                    <p className="text-white">{selectedEmployee.department?.name || 'Unassigned'}</p>
                  </div>
                  {selectedEmployee.manager && (
                    <div>
                      <span className="text-gray-400 text-sm">Manager:</span>
                      <p className="text-white">{selectedEmployee.manager.name}</p>
                    </div>
                  )}
                  {selectedEmployee.directReports && selectedEmployee.directReports.length > 0 && (
                    <div>
                      ...
                      <p className="text-white">{selectedEmployee.directReports.length} direct reports</p>
                    </div>
                  )}
                </CardContent>
              </Card>
                  ...
                  <CardContent className="space-y-4">
                    {selectedEmployee.skills && (
                      <div>
                        ...
                        <p className="text-white">{selectedEmployee.skills}</p>
                      </div>
                    )}
                    {selectedEmployee.bio && (
                      <div>
                        <span className="text-gray-400 text-sm block mb-2">Bio:</span>
                        <p className="text-white">{selectedEmployee.bio}</p>
                      </div>
                    )}
                  </CardContent>
                </Card>
